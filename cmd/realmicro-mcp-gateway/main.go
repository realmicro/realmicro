// Command realmicro-mcp-gateway runs a standalone MCP gateway that discovers
...
//
//	# etcd
//	realmicro-mcp-gateway --address :3000 --registry etcd --registry-address 127.0.0.1:2379
package main
...
import (
	...
	"github.com/realmicro/realmicro/ai/mcp"
...
func main() {
	app := &cli.App{
		Name:    "realmicro-mcp-gateway",
		Usage:   "Standalone MCP gateway for realmicro services",
		Version: version,
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:  "address",
			...
			&cli.StringFlag{
				Name:    "registry",
				Usage:   "Service registry (etcd)",
				Value:   "etcd",
				EnvVars: []string{"REALMICRO_REGISTRY"},
			},
			&cli.StringFlag{
				Name:    "registry-address",
				Usage:   "Registry address (e.g., etcd:2379)",
				EnvVars: []string{"REALMICRO_REGISTRY_ADDRESS"},
			},
			&cli.Float64Flag{
				Name:    "rate-limit",
...
func newRegistry(name, address string) (registry.Registry, error) {
	...
	switch name {
	case "etcd":
		return etcd.NewRegistry(opts...), nil
	default:
		return nil, fmt.Errorf("unknown registry %q (supported: etcd)", name)
	}
}
